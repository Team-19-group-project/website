<!DOCTYPE html>
<html>
    <head>
	    <meta charset="utf-8">
	    <title>About Us</title>
		<!-- Loads the css code needed to add style to each page, changing this file would have an impact on all pages-->
	    <link href="css/style.css" rel="stylesheet">
	</head>
    <body>
		<h1 class="game_Title" align="center">+Three</h1>
		<!-- Navigation bar provides the ability to change pages-->
	    <div class="navbar">		
        <ul>
        <li><a href="index.php">Home</a></li>
        <li><a href="Register.php">Register</a></li>
		<li><a href="Login.php">Login</a></li>
		<li><a class="active" href="about.php">About Us</a></li>
		<li><a href="download.php">Download</a></li>
        </ul>
		</div>
		<h1 class="aston_Title" align="center"> About Us</h1>
		
		<h1 class="aston_Title" align="center"> Hover over the images to learn more about us</h1>
		
		<p><center> We are a group of students from Aston University that have created an exciting card game for our team project that has been in development since September</center><p>
		
		<!--Card template used from w3schools.com-->
		<div class = "row"> 
			<div class = "column"> 
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Jeevan.PNG" alt="Image of Jeevan Saini" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Jeevan Saini</h1> 
							<p>Website Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Jabbi.jpeg" alt="Image of Banta Jabbi Jabbie" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Banta Jabbi Jabbie</h1> 
							<p>Game Development</p> 
							<p>Bsc Computer Science with Business</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Seb.jpeg" alt="Image of Sebastian Jacques" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Sebastian Jacques</h1> 
							<p>Game Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Zain.jpeg" alt="Image of Zain Mahmood" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Zain Mahmood</h1> 
							<p>Game Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Alex.png" alt="Image of Alex Abrahall" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Alex Abrahall</h1> 
							<p>Game Development</p> 
							<p>Bsc Computer Science with Business</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Husna.jpeg" alt="Image of Husna Madessar" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Husna Madessar</h1> 
							<p>Website Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Talha.png" alt="Image of Talha Khalid" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Talha Khalid</h1> 
							<p>Game Development</p> 
							<p>Bsc Computer Science with Business</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Iram.png" alt="Image of Iram Mahbub" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Iram Mahbub</h1> 
							<p>Game Development</p> 
							<p>Bsc Computer Science with Business</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Chidera.png" alt="Image of Chidera Nwagbolu" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Chidera Nwagbolu</h1> 
							<p>Website Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
			
			<div class = "column">
				<div class="flip-card">
					<div class="flip-card-inner">
						<div class="flip-card-front">
							<img src="Images/img_Waleed.jpeg" alt="Image of Muhammed Waleed" style="width:250px;height:250px;">
						</div>
						<div class="flip-card-back">
							<h1>Muhammed Waleed</h1> 
							<p>Game Development</p> 
							<p>Bsc Cybersecurity</p>
						</div>
					</div>
				</div>
			</div>
		</div>
    </body>
</html>

